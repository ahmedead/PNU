...
{
    /// <summary>
    /// Idempotent C# provisioner for Professional Certifications SharePoint lists on /ar/Agencies/AcademicAffairs.
    /// Creates missing lists, adds columns, and seeds default data from data.xlsx on initial creation
    /// or when manually requested via the admin control.
    /// </summary>
    public static class PcListProvisioner
    {
        ...
        {
            if (PcListSchema.Get(listName) == null) return;
            if (DoneThisRequest("PcList:" + listName)) return;
            ...
            try
            {
                ...
                MarkDoneThisRequest("PcList:" + listName);
            }
            catch (Exception ex)
            {
                PcLog.Write("PcListProvisioner.EnsureListExists:" + listName, ex);
            }
        }

        public static void EnsureAllListsExist()
        {
            if (DoneThisRequest("PcAllLists")) return;
            ...
            try
            {
                ...
                using (SPWeb web = PcTargetWeb.Open(site))
                {
                    ...
                    foreach (string listName in PcListNames.All)
                    {
                        EnsureOnResolvedWeb(site, web, listName);
                        MarkDoneThisRequest("PcList:" + listName);
                    }
                ...
                MarkDoneThisRequest("PcAllLists");
            }
            catch (Exception ex)
            {
                PcLog.Write("PcListProvisioner.EnsureAllListsExist", ex);
            }
        }
        ...
        private static bool DoneThisRequest(string key)
        {
            try
            {
                ...
                return ctx != null && ctx.Items.Contains("PcProv:" + key);
            }
            catch { return false; }
        }
        ...
        private static void MarkDoneThisRequest(string key)
        {
            ...
            {
                System.Web.HttpContext ctx = System.Web.HttpContext.Current;
                if (ctx != null) ctx.Items["PcProv:" + key] = true;
            }
            catch { }
        }
        ...
        public static void EnsureListOnWeb(Guid siteId, Guid webId, PcListDef def)
        {
            if (def == null) return;
            ...
            {
                PcLog.Write("PcListProvisioner.EnsureListOnWeb:" + def.Name, ex);
            }
        }

        /// <summary>
        /// Manually forces re-seeding of default schema data into the specified list.
        /// Deletes existing items and re-inserts fresh seed data from data.xlsx.
        /// </summary>
        public static bool ForceSeedDefaultData(string listName)
        {
            ...
            {
                PcLog.Write("PcListProvisioner.ForceSeedDefaultData:" + listName, ex);
                success = false;
            }
        ...
        private static void EnsureColumns(SPList list, PcListDef def)
        {
            bool changed = false;

            foreach (PcFieldDef f in def.Fields)
            {
                if (!list.Fields.ContainsField(f.InternalName))
                {
                    string displayName = PcHelper.Pick(f.DisplayAr, f.DisplayEn);
        ...
        private static void SeedDefaultData(SPList list, PcListDef def)
        {
            ...
            {
                PcLog.Write("PcListProvisioner.SeedDefaultData:" + def.Name, ex);
            }
        }
    }
}
