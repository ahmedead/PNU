...
{
    public partial class ucFacilitesDetails : UserControl
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (Page.Request.QueryString["FacilityId"] == null)
                    return;
                GetData();
        ...
        private bool GetData()
        {
            ...
            try
            {
                ...
                var RequestId = Convert.ToInt32(Page.Request.QueryString["FacilityId"]);
                ...
                {
                    {
                        using (SPSite site = new SPSite(SPContext.Current.Web.Url))
                        {
                            using (SPWeb web = site.OpenWeb())
                            {
                                ...
                                SPList FacilitiesServicesList = web.Lists["FacilitiesServices"];
                                ...
                                SPListItemCollection items = FacilitiesServicesList.GetItems(query);
