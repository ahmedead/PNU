...
{
    /// <summary>
    /// Idempotent C# provisioner for Central Library SharePoint lists on /ar/UniversityLife.
    /// Creates missing lists, adds columns, and seeds default HTML data on initial creation
    /// or when manually requested via the admin control.
    /// </summary>
    public static class ClListProvisioner
    {
        ...
        {
            if (ClListSchema.Get(listName) == null) return;
            if (DoneThisRequest("ClList:" + listName)) return;
            ...
            try
            {
                ...
                MarkDoneThisRequest("ClList:" + listName);
            }
            catch (Exception ex)
            {
                ClLog.Write("ClListProvisioner.EnsureListExists:" + listName, ex);
            }
        }

        public static void EnsureAllListsExist()
        {
            if (DoneThisRequest("ClAllLists")) return;
            ...
            try
            {
                ...
                using (SPWeb web = ClTargetWeb.Open(site))
                {
                    ...
                    foreach (string listName in ClListNames.All)
                    {
                        EnsureOnResolvedWeb(site, web, listName);
                        MarkDoneThisRequest("ClList:" + listName);
                    }
                ...
                MarkDoneThisRequest("ClAllLists");
            }
            catch (Exception ex)
            {
                ClLog.Write("ClListProvisioner.EnsureAllListsExist", ex);
            }
        }
        ...
        private static bool DoneThisRequest(string key)
        {
            try
            {
                ...
                return ctx != null && ctx.Items.Contains("ClProv:" + key);
            }
            catch { return false; }
        }
        ...
        private static void MarkDoneThisRequest(string key)
        {
            ...
            {
                System.Web.HttpContext ctx = System.Web.HttpContext.Current;
                if (ctx != null) ctx.Items["ClProv:" + key] = true;
            }
            catch { }
        }
        ...
        public static void EnsureListOnWeb(Guid siteId, Guid webId, ClListDef def)
        {
            if (def == null) return;
            ...
            {
                ClLog.Write("ClListProvisioner.EnsureListOnWeb:" + def.Name, ex);
            }
        }
        ...
        public static bool ForceSeedDefaultData(string listName)
        {
            ...
            {
                ClLog.Write("ClListProvisioner.ForceSeedDefaultData:" + listName, ex);
                success = false;
            }
        ...
        private static void EnsureColumns(SPList list, ClListDef def)
        {
            bool changed = false;
            ...
            foreach (ClFieldDef f in def.Fields)
            {
                ...
                {
                    string displayName = ClHelper.Pick(f.DisplayAr, f.DisplayEn);
        ...
        private static void SeedDefaultData(SPList list, ClListDef def)
        {
            ...
            {
                ClLog.Write("ClListProvisioner.SeedDefaultData:" + def.Name, ex);
            }
        }
    }
}
