...
{
    /// <summary>
    /// Resolves the web that holds the Central Library SharePoint lists (/ar/CentralLibrary).
    /// </summary>
    public static class ClTargetWeb
    {
        public const string DefaultPath = "/ar/UniversityLife";
        ...
        public static void EnsureTargetWebExists(SPSite site)
        {
            ...
            try
            {
                ...
                // If subweb does not exist, attempt to create it under elevated privileges
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    using (SPSite elevatedSite = new SPSite(site.ID))
                    {
                        string targetPath = Path.Trim('/'); // e.g. "ar/CentralLibrary"
                        try
                        {
                            ...
                            using (SPWeb newWeb = elevatedSite.RootWeb.Webs.Add(
                                targetPath,
                                "المكتبة المركزية",
                                "Central Library Web for DGA Lists",
                                1025,
                                "STS#0",
                                false,
                                false))
                        ...
                        {
                            ClLog.Write("ClTargetWeb.EnsureTargetWebExists", ex);
                        }
                    }
                });
            }
            ...
            {
                ClLog.Write("ClTargetWeb.EnsureTargetWebExists.Outer", ex);
            }
        }
        ...
        private static SPWeb OpenPath(SPSite site, string path, bool allowFallback)
        {
            ...
            {
                ClLog.Write("ClTargetWeb.OpenFallback", ex);
            }
