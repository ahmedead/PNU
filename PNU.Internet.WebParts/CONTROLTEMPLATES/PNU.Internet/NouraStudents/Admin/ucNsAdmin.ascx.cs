...
{
    /// <summary>
    /// Generic add / edit / delete control for every Noura Students list.
    /// The form is generated from NsListSchema, so adding a field to the schema
    ...
    /// </summary>
    public partial class ucNsAdmin : UserControl
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            {
                NsLog.Write("ucNsAdmin.OnInit", ex);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                NsLog.Write("ucNsAdmin.Page_Load", ex);
                ShowError(ex.Message);
            }
        }
            ...
            get
            {
                try
                {
                    if (Page != null && Page.IsPostBack)
                    {
                        ...
                        if (!string.IsNullOrEmpty(posted) && NsListSchema.Get(posted) != null)
                            return posted;
                    }
                }
        ...
        {
            ltPageTitle.Text = NsHelper.Enc(NsHelper.GetRes("Ns_AdminTitle",
                "إدارة محتوى طالبات نورة", "Noura Students content management"));
            lblListLabel.Text = NsHelper.Enc(NsHelper.GetRes("Ns_AdminList", "القسم", "Section"));
            ltNew.Text = NsHelper.Enc(NsHelper.GetRes("Ns_AdminNew", " إضافة عنصر", " Add item"));
            ltActions.Text = NsHelper.Enc(NsHelper.GetRes("Ns_AdminActions", "إجراءات", "Actions"));
            ltEmpty.Text = NsHelper.Enc(NsHelper.GetRes("Ns_AdminEmpty",
                "لا توجد عناصر في هذا القسم بعد.", "No items in this section yet."));
            ltDenied.Text = NsHelper.Enc(NsHelper.GetRes("Ns_AdminDenied",
                "ليس لديك صلاحية لإدارة هذا المحتوى.", "You do not have permission to manage this content."));

            btnSave.Text = NsHelper.Enc(NsHelper.GetRes("Ns_AdminSave", "حفظ", "Save"));
            btnCancel.Text = NsHelper.Enc(NsHelper.GetRes("Ns_AdminCancel", "إلغاء", "Cancel"));
        }
        ...
        private void FillListPicker()
        {
            ...
            {
                NsListDef only = NsListSchema.Get(FixedListName);
                if (only != null)
                {
            ...
            foreach (string name in NsListNames.AllLists)
            {
                NsListDef def = NsListSchema.Get(name);
        ...
        private NsListDef CurrentDef { get { return NsListSchema.Get(CurrentListName); } }
        ...
        private void BuildForm(string listName)
        {
            ...
            AddInput("Title", NsHelper.Pick("العنوان (عربي)", "Title (AR)"), SPFieldType.Text, 0, null, null);
                              ...
                              int rows, string hint, string[] choices)
        {
            ...
            if (type == SPFieldType.Boolean)
            {
                ...
                flag.Items.Add(new ListItem(NsHelper.Pick("نعم", "Yes"), "1"));
                flag.Items.Add(new ListItem(NsHelper.Pick("لا", "No"), "0"));
            ...
            if (type == SPFieldType.DateTime && string.IsNullOrEmpty(hint))
                hint = NsHelper.Pick("الصيغة: 2026-06-24", "Format: 2026-06-24");

            if (!string.IsNullOrEmpty(hint))
                col.Controls.Add(new Literal { Text = "<div class=\"form-text\">" + NsHelper.Enc(hint) + "</div>" });
        ...
        private void RestoreFormState()
        {
            ...
            ltFormTitle.Text = NsHelper.Enc(mode == ModeEdit
                ? NsHelper.GetRes("Ns_AdminEdit", "تعديل عنصر", "Edit item")
                : NsHelper.GetRes("Ns_AdminNew", "إضافة عنصر", "Add item"));
        }
        ...
        private void LoadItemIntoForm(int id)
        {
            try
            {
                ...
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    ...
                    using (SPWeb web = NsTargetWeb.Open(site))
                    {
                        ...
                        SetValue("Title", NsHelper.SafeString(item, "Title"));
                        ...
                        foreach (NsFieldDef f in def.Fields)
                        {
                            ...
                            {
                                value = NsHelper.SafeUrl(item, f.InternalName);
                            }
                            else if (f.Type == SPFieldType.DateTime)
                            {
                                DateTime? d = NsHelper.SafeDate(item, f.InternalName);
                                value = d.HasValue
                            ...
                            {
                                value = NsHelper.SafeBool(item, f.InternalName) ? "1" : "0";
                            }
                            else
                            {
                                value = NsHelper.SafeString(item, f.InternalName);
                            }
            ...
            {
                NsLog.Write("ucNsAdmin.LoadItemIntoForm", ex);
                ShowError(ex.Message);
            }
        }
        ...
        private void SaveItem()
        {
            ...
            {
                ShowError(NsHelper.GetRes("Ns_AdminTitleRequired", "العنوان مطلوب.", "Title is required."));
                RestoreFormState();
                return;
            }
            ...
            try
            {
                ...
                BindGrid();
                ShowMessage(isNew
                    ? NsHelper.GetRes("Ns_AdminAdded", "تمت إضافة العنصر بنجاح.", "Item added successfully.")
                    : NsHelper.GetRes("Ns_AdminUpdated", "تم تحديث العنصر بنجاح.", "Item updated successfully."));
            }
            catch (Exception ex)
            {
                NsLog.Write("ucNsAdmin.SaveItem", ex);
                ShowError(ex.Message);
                RestoreFormState();
            }
        }
        ...
        private void DeleteItem(int id)
        {
            ...
            try
            {
                ...
                CloseForm();
                BindGrid();
                ShowMessage(NsHelper.GetRes("Ns_AdminDeleted", "تم حذف العنصر.", "Item deleted."));
            }
            catch (Exception ex)
            {
                NsLog.Write("ucNsAdmin.DeleteItem", ex);
                ShowError(ex.Message);
            }
        }
        ...
        private void BindGrid()
        {
            ...
            var headers = new List<string> { NsHelper.Enc(NsHelper.Pick("العنوان (عربي)", "Title (AR)")) };
            var gridFields = def.Fields.Where(f => f.InGrid).ToList();
            foreach (NsFieldDef f in gridFields) headers.Add(NsHelper.Enc(f.Display));

            rptHeader.DataSource = headers;
            rptHeader.DataBind();

            string editText = NsHelper.Enc(NsHelper.GetRes("Ns_AdminEditBtn", "تعديل", "Edit"));
            string deleteText = NsHelper.Enc(NsHelper.GetRes("Ns_AdminDeleteBtn", "حذف", "Delete"));
            string confirmText = NsHelper.GetRes("Ns_AdminConfirm",
                "هل أنت متأكد من حذف هذا العنصر؟", "Are you sure you want to delete this item?")
                .Replace("'", "\\'");

            var rows = new List<NsAdminRow>();
            ...
            try
            {
                ...
                using (SPWeb web = NsTargetWeb.Open(site))
                {
                    ...
                    foreach (SPListItem item in NsHelper.GetItems(web, def.Name))
                    {
                        var row = new NsAdminRow
                        {
                            Id = item.ID,
                            EditText = editText,
                        ...
                        row.Cells.Add(NsHelper.Enc(NsHelper.SafeString(item, "Title")));

                        foreach (NsFieldDef f in gridFields)
                        {
                            string value;
                            ...
                            row.Cells.Add(NsHelper.Enc(value));
                        }
            ...
            {
                NsLog.Write("ucNsAdmin.BindGrid:" + def.Name, ex);
            }
        ...
        {
            ltMessage.Text = NsHelper.Enc(text);
            pnlMessage.Visible = true;
        }

        private void ShowError(string text)
        {
            ltError.Text = NsHelper.Enc(text);
        ...
        private bool IsAdmin()
        {
            ...
            {
                NsLog.Write("ucNsAdmin.IsAdmin", ex);
                return false;
            }
        }
        ...
        private bool IsListedAdmin(SPWeb web, int userId, string userName)
        {
            ...
            try
            {
                ...
                {
                    string active = NsHelper.SafeString(item, "Active").Trim();
            ...
            {
                NsLog.Write("ucNsAdmin.IsListedAdmin:" + web.Url, ex);
            }
    ...
    public class NsAdminRow
    {
        ...
        public NsAdminRow() { Cells = new List<string>(); }
    }
}
