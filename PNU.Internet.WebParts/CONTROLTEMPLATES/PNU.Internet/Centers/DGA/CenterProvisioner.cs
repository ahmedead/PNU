...
namespace PNU.Internet.WebParts.CONTROLTEMPLATES.PNU.Internet.Centers.DGA
{
    public static class CenterProvisioner
    {
        ...
        public static void GrantAnonymousRead(SPList list)
        {
            ...
            {
                Publics.WriteToLog(HttpContext.Current?.Request?.Url?.ToString() ?? "", "CenterProvisioner.GrantAnonymousRead", ex.Message);
            }
        }

        private static void RunElevatedProvisioning(SPWeb web, Action<SPWeb> action, string logContext)
        {
            ...
            lock (_lock)
            {
                try
                {
                    ...
                    SPSecurity.RunWithElevatedPrivileges(() =>
                    {
                        ...
                        {
                            bool prevUnsafe = elevatedWeb.AllowUnsafeUpdates;
                            ...
                            {
                                elevatedWeb.AllowUnsafeUpdates = prevUnsafe;
                            }
                        }
                    });
                }
                ...
                {
                    Publics.WriteToLog(HttpContext.Current?.Request?.Url?.ToString() ?? "", logContext, ex.Message);
                }
            }
        }
        ...
        {
            RunElevatedProvisioning(web, elevatedWeb =>
            {
                EnsureDepartmentsListInternal(elevatedWeb, "CenterDepartments");
                EnsureBeneficiariesListInternal(elevatedWeb, "CenterBeneficiaries");
                EnsureRecordListInternal(elevatedWeb, "CenterServiceRecord");
                EnsureProgramsListInternal(elevatedWeb, "CenterPrograms");
                EnsureDigitalChannelsListInternal(elevatedWeb, "CenterDigitalChannels");
                EnsureHomeContentListsInternal(elevatedWeb);
                EnsureContactListsInternal(elevatedWeb);
                EnsureDocumentsListInternal(elevatedWeb, "CenterDocuments");
            }, "CenterProvisioner.EnsureAllLists");
        }
        ...
        public static void EnsureDepartmentsList(SPWeb web, string listName = "CenterDepartments")
        {
            ...
            RunElevatedProvisioning(web, elevatedWeb =>
            {
                EnsureDepartmentsListInternal(elevatedWeb, targetListName);
            }, "CenterProvisioner.EnsureDepartmentsList");
        }

        private static void EnsureDepartmentsListInternal(SPWeb web, string listName)
        {
        ...
        public static void EnsureBeneficiariesList(SPWeb web, string listName = "CenterBeneficiaries")
        {
            ...
            RunElevatedProvisioning(web, elevatedWeb =>
            {
                EnsureBeneficiariesListInternal(elevatedWeb, targetListName);
            }, "CenterProvisioner.EnsureBeneficiariesList");
        }

        private static void EnsureBeneficiariesListInternal(SPWeb web, string listName)
        {
            SPList list = web.Lists.TryGetList(listName);
            bool created = false;
            if (list == null)
        ...
        public static void EnsureRecordList(SPWeb web, string listName = "CenterServiceRecord")
        {
            ...
            RunElevatedProvisioning(web, elevatedWeb =>
            {
                EnsureRecordListInternal(elevatedWeb, targetListName);
            }, "CenterProvisioner.EnsureRecordList");
        }

        private static void EnsureRecordListInternal(SPWeb web, string listName)
        {
            SPList list = web.Lists.TryGetList(listName);
            bool created = false;
            if (list == null)
        ...
        public static void EnsureProgramsList(SPWeb web, string listName = "CenterPrograms")
        {
            ...
            RunElevatedProvisioning(web, elevatedWeb =>
            {
                EnsureProgramsListInternal(elevatedWeb, targetListName);
            }, "CenterProvisioner.EnsureProgramsList");
        }

        private static void EnsureProgramsListInternal(SPWeb web, string listName)
        {
            SPList list = web.Lists.TryGetList(listName);
            bool created = false;
            if (list == null)
        ...
        public static void EnsureDigitalChannelsList(SPWeb web, string listName = "CenterDigitalChannels")
        {
            ...
            RunElevatedProvisioning(web, elevatedWeb =>
            {
                EnsureDigitalChannelsListInternal(elevatedWeb, targetListName);
            }, "CenterProvisioner.EnsureDigitalChannelsList");
        }

        private static void EnsureDigitalChannelsListInternal(SPWeb web, string listName)
        {
            SPList list = web.Lists.TryGetList(listName);
            bool created = false;
            if (list == null)
        ...
        public static void EnsureHomeContentLists(SPWeb web)
        {
            ...
            {
                EnsureHomeContentListsInternal(elevatedWeb);
            }, "CenterProvisioner.EnsureHomeContentLists");
        }

        private static void EnsureHomeContentListsInternal(SPWeb web)
        {
            // 1. CenterHomeContent
            ...
            if (contentList == null)
            {
                ...
                contentList.OnQuickLaunch = false;
                contentList.Update();
            }
            ...
            contentList.Update();
            GrantAnonymousRead(contentList);
        ...
        public static void EnsureContactLists(SPWeb web)
        {
            ...
            {
                EnsureContactListsInternal(elevatedWeb);
            }, "CenterProvisioner.EnsureContactLists");
        }

        private static void EnsureContactListsInternal(SPWeb web)
        {
        ...
        public static void EnsureDocumentsList(SPWeb web, string listName = "CenterDocuments")
        {
            ...
            RunElevatedProvisioning(web, elevatedWeb =>
            {
                EnsureDocumentsListInternal(elevatedWeb, targetListName);
            }, "CenterProvisioner.EnsureDocumentsList");
        }

        private static void EnsureDocumentsListInternal(SPWeb web, string listName)
        {
            SPList list = web.Lists.TryGetList(listName);
            if (list == null)
            {
