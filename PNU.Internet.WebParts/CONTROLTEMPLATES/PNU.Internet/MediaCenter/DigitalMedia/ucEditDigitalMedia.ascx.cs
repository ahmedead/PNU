...
{
    public partial class ucEditDigitalMedia : UserControl
    {
            ...
            {
                {
                    using (SPSite site = new SPSite(SPContext.Current.Site.ID))
                    {
                        using (SPWeb web = site.OpenWeb("/ar/MediaCenter/News/"))
                        {
                            ...
                            if(item["MainCategory_EN"] != null)
                                ddlMediaCategory.SelectedValue = item["MainCategory_EN"].ToString();
                            ...
                            if (item["PublisherName"] != null)
                                txtPublisherName.Text = item["PublisherName"].ToString();
                            if (item["PublisherPosition"] != null)
                                txtPublisherPosition.Text = item["PublisherPosition"].ToString() ;



                            if (item["VideoURL"] != null)
                            {
                                string fileUrl = item["VideoURL"].ToString();

                                // Bind the URL to a hyperlink for the user to see/download the file
                                HyperLinkFile.NavigateUrl = fileUrl;
                                HyperLinkFile.Text = "ملف الفيديو"; // Display text
                                HyperLinkFile.Visible = true;

                                // Alternatively, store the URL in a hidden field if needed
                                HiddenFieldFileUrl.Value = fileUrl;
                        ...
                        using (SPSite site = new SPSite(SPContext.Current.Site.ID))
                        {
                            using (SPWeb web = site.OpenWeb("/ar/MediaCenter/News/"))
                            {
                                ...
                                if (chkRemoveImg.Checked)
                                {
                                    ...
                                    {
                                        // Iterate through all attachments and delete them
                                        for (int i = listItem.Attachments.Count - 1; i >= 0; i--)
                                        {
                                            string attachmentName = listItem.Attachments[i];
                                            listItem.Attachments.Delete(attachmentName);  // Delete each attachment by name
                                        }
                                    }
                                }
                                ...
                                if (VideofileUPload.PostedFile != null && VideofileUPload.HasFile)
                                {
                                    ...
                                    string folderName = "DigitalMedia"; // Replace with the actual ID you provide
