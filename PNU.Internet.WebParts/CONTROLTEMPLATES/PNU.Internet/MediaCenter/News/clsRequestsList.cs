...
namespace PNU.Internet.WebParts.CONTROLTEMPLATES.PNU.Internet.MediaCenter.News
{
    public class clsRequestsList
    {
        ...
        public string DetailsURL
        {
            get
            {
                if(FacultyName_EN == null || FacultyName_EN == "")
                {
                    ...
                    {
                        if (PortalHelper.IsArabic)
                            return "/ar/MediaCenter/News/Pages/DigitalMediaDetails.aspx?RequestID=" + ID;
                        else
                    ...
                    {
                        if (PortalHelper.IsArabic)
                            return "/ar/MediaCenter/News/Pages/NewsDetails.aspx?RequestID=" + ID;
                        else
        ...
        // ============================================================
        // ADD THESE PROPERTIES INSIDE your existing clsRequestsList
        // (do NOT create a new class — just paste these inside it)
        // ============================================================

        // --- NEW: SearchCategory field (separate from MainCategory) ---
        // Make sure your SharePoint list has a column named "SearchCategory"
        // (or a Lookup / Taxonomy column) and that busclsRequestsList maps it
        // into these properties. If the internal name differs, adjust the mapping
        // in busclsRequestsList accordingly.
        public string SearchCategory { get; set; } = "";
        public string SearchCategory_EN { get; set; } = "";

        public string DisplaySearchCategory
        {
            get
            {
                if (PortalHelper.IsArabic)
                    return string.IsNullOrEmpty(SearchCategory) ? "" : SearchCategory;
                else
                    return string.IsNullOrEmpty(SearchCategory_EN) ? "" : SearchCategory_EN;
            }
        }

        // --- Already added previously for the new designs ---

        // ISO date (yyyy-MM-dd) for the <time datetime="..."> attribute in the new design
        public string MediaDateISO
        {
            get
            {
                if (_MediaDate == null) return string.Empty;
                try
                {
                    DateTime dt = Convert.ToDateTime(_MediaDate.ToString());
        ...
        public string CreatedDateISO
        {
            get
            {
                ...
                try
                {
                    DateTime dt = Convert.ToDateTime(Created.ToString());
                    return dt.ToString("yyyy-MM-dd");
                }
                catch { return string.Empty; }
            }
        ...
        // Localized media type label (صورة / فيديو) for the badge text
        public string MediaTypeLabel
        {
            get
            {
                if (PortalHelper.IsArabic)
                    return string.IsNullOrEmpty(MediaTypes) ? "" : MediaTypes;
                else
                    return string.IsNullOrEmpty(MediaTypes_EN) ? "" : MediaTypes_EN;
            }
        }

        // CSS class for the media type badge — badge-success for video, badge-info for image
        public string MediaBadgeClass
        {
            get
            {
                if (IsVideo) return "badge-success";
                return "badge-info";
            }
        }
    ...
    public class clsAdvertisements
    {
        public string ID { get; set; } = "";
        public string Title { get; set; } = "";
        public string Title_EN { get; set; } = "";
        ...
        // ============================================================
        // ADD THESE PROPERTIES INSIDE your existing clsAdvertisements
        // (do NOT create a new class — just paste these inside it)
        // ============================================================

        // --- NEW: SearchCategory field ---
        // Add a column named "SearchCategory" (and optionally "SearchCategory_EN")
        // to the AdvertisementsRequests SharePoint list. Sample values:
        // التدريب والفرص / Training & Opportunities
        // القبول والتسجيل / Admission & Registration
        // الخدمات الإلكترونية / E-Services
        // أكاديمي / Academic
        // الطالبات / Students
        // Make sure busclsAdvertisements (or however your mapper is named) maps
        // these columns into the properties below.
        public string SearchCategory { get; set; } = "";
        public string SearchCategory_EN { get; set; } = "";
        ...
        // ISO date (yyyy-MM-dd) for <time datetime="..."> attribute
        public string MediaDateISO
        {
            get
            {
                if (string.IsNullOrEmpty(_MediaDate)) return string.Empty;
                try
                {
                    DateTime dt = Convert.ToDateTime(_MediaDate);
                    return dt.ToString("yyyy-MM-dd");
                }
                catch { return string.Empty; }
            }
        ...
        // Localized day name (الأحد, الإثنين, ... / Sunday, Monday, ...)
        // Used for the small pill at the top of each event card.
        public string DayName
        {
            get
            {
                ...
                {
                    DateTime dt = Convert.ToDateTime(_MediaDate);
                    string lang = PortalHelper.IsArabic ? "ar-AE" : "en-US";
                    return dt.ToString("dddd", new System.Globalization.CultureInfo(lang));
                }
                catch { return string.Empty; }
            }
        }
    ...
    public class clsAllRequestsList
    {
        public string ID { get; set; }
        public string MainCategory { get; set; }
        public string MainCategory_EN { get; set; }
        public List<clsRequestsList> Requests { get; set; }
        

        public List<dataEvent> Events { get; set; }

        public List<clsAdvertisements> Advertisements { get; set; }
    ...
    public static class busclsRequestsList
    {
        ...
        public static string SiteURL = "/ar/MediaCenter/News/";
        public static clsRequestsList GetItemByID(string ID)
        {
            try
            {
                ...
                if (objNew == null)
                    return null;
                clsRequestsList _Data = SPFactory.MapListItemsToClass<clsRequestsList>(objNew);
                return _Data;
        ...
        public static List<clsLookUP> GetAllItemsLookup(string ListName)
        {
            try
            {
                ...
                return SPFactory.GetAllDataByQuery<clsLookUP>(SiteURL, ListName, query);
        ...
        public static List<clsRequestsList> GetAllItems( int RowLimit = 0)
        {
            try
            {
                SPQuery query = new SPQuery();
                ...
                //else
                //    query.RowLimit = 20;
                return SPFactory.GetAllDataByQuery<clsRequestsList>(SiteURL, RequestsListName, query);
        ...
        public static List<clsRequestsList> GetAllItemsIsHome(int RowLimit = 0)
        {
            try
            {
                SPQuery query = new SPQuery();
        ...
        public static List<clsRequestsList> GetAllItemsIsHomePending(int RowLimit = 0)
        {
            try
            {
                SPQuery query = new SPQuery();
        ...
        public static List<clsRequestsList> GetCollegeNews(string CollegeCode,int RowLimit = 0)
        {
            try
            {
                SPQuery query = new SPQuery();
        ...
        public static List<clsRequestsList> GetAllItemsByMediaCategory(string MediaCategory,int RowLimit = 0)
        {
            try
            {
                SPQuery query = new SPQuery();
        ...
        public static List<clsRequestsList> GetAllItemsByMediaCategoryNotMainCat(int RowLimit = 0)
        {
            try
            {
                ...
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    using (SPSite site = new SPSite(SPContext.Current.Site.ID))
                    {
                        using (SPWeb web = site.OpenWeb(SiteURL))
                        {
                            ...
                            {
                                _returnList = SPFactory.MapListItemsToClass<clsRequestsList>(objNew);
                            }
                        }
                    }
                });
