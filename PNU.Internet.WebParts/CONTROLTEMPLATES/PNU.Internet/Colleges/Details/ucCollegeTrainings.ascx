...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ucCollegeTrainings.ascx.cs" Inherits="PNU.Internet.WebParts.CONTROLTEMPLATES.PNU.Internet.Colleges.Details.ucCollegeTrainings" %>
...
<section id="faculty-training" class="pnu-section-anchor">
    ...
    <div class="accordion accordion-flush mt-3" id="faculty-trainingAccordion">
        <asp:Repeater ID="rptStudentTraining" runat="server" OnItemDataBound="rptStudentTraining_ItemDataBound">
            <ItemTemplate>
                <div class="accordion-item">
                    <p class="accordion-header" id='faculty-trainingAccordionHeading<%# Container.ItemIndex + 1 %>'>
                        <button
                            class='accordion-button <%# Container.ItemIndex == 0 ? "" : "collapsed" %>'
                            type="button"
                            data-bs-toggle="collapse"
                            data-bs-target='#faculty-trainingAccordionCollapse<%# Container.ItemIndex + 1 %>'
                            aria-expanded='<%# Container.ItemIndex == 0 ? "true" : "false" %>'
                            aria-controls='faculty-trainingAccordionCollapse<%# Container.ItemIndex + 1 %>'>
                            <%# Eval("Title") %>
                        </button>
                    </p>
                    <div id='faculty-trainingAccordionCollapse<%# Container.ItemIndex + 1 %>'
                         class='accordion-collapse collapse <%# Container.ItemIndex == 0 ? "show" : "" %>'
                         aria-labelledby='faculty-trainingAccordionHeading<%# Container.ItemIndex + 1 %>'
