...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ucCollegeInitiatives.ascx.cs" Inherits="PNU.Internet.WebParts.CONTROLTEMPLATES.PNU.Internet.Colleges.Details.ucCollegeInitiatives" %>
...
<section id="faculty-initiatives" class="pnu-section-anchor mb-5">
    ...
    <div class="accordion accordion-flush mt-3" id="faculty-initiativesAccordion">
        <asp:Repeater ID="rptStudentInitiatives"
            runat="server"
            OnItemDataBound="rptStudentInitiatives_ItemDataBound">
            <ItemTemplate>
                <div class="accordion-item">
                    <p class="accordion-header"
                       id='faculty-initiativesAccordionHeading<%# Container.ItemIndex + 1 %>'>
                        <button
                            class='accordion-button <%# Container.ItemIndex == 0 ? "" : "collapsed" %>'
                            type="button"
                            data-bs-toggle="collapse"
                            data-bs-target='#faculty-initiativesAccordionCollapse<%# Container.ItemIndex + 1 %>'
                            aria-expanded='<%# Container.ItemIndex == 0 ? "true" : "false" %>'
                            aria-controls='faculty-initiativesAccordionCollapse<%# Container.ItemIndex + 1 %>'>
                            <%# Eval("Title") %>
                        </button>
                    </p>
                    <div id='faculty-initiativesAccordionCollapse<%# Container.ItemIndex + 1 %>'
                         class='accordion-collapse collapse <%# Container.ItemIndex == 0 ? "show" : "" %>'
                         aria-labelledby='faculty-initiativesAccordionHeading<%# Container.ItemIndex + 1 %>'
