...
namespace PNU.Internet.SearchIndex.Tasks
{
    ...
    public static class IndexingTasks
    {
        ...
        public const string TASK_RETRY_FAILED = "RetryFailed";
        ...
        // ================================================================
        public static TaskResult RunRetryFailedTask(Guid siteId)
        {
            var result = new TaskResult { TaskName = TASK_RETRY_FAILED };
            var errors = new StringBuilder();

            try
            {
                int n = SearchIndexer.ProcessRetries(siteId, errors);
                result.ItemsCount = n;
                result.WebsTouched = 1;
            }
            catch (Exception ex)
            {
                errors.AppendLine("FATAL: " + ex.Message);
                SearchLogger.WriteToLog("Tasks",
                    "RunRetryFailedTask " + siteId, ex.Message);
            }
